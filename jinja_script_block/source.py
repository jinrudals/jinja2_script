...
def rewrite_script_blocks(source, environment, name, filename=None):
    ...
    block_start, block_end = environment.block_start_string, environment.block_end_string
    starts = (block_start, environment.variable_start_string, environment.comment_start_string)
    ends = (block_end, environment.variable_end_string, environment.comment_end_string)
    close_script = re.compile(re.escape(block_start) + r'[+-]?\s*endscript\s*[+-]?' + re.escape(block_end))
    close_raw = re.compile(re.escape(block_start) + r'[+-]?\s*endraw\s*[+-]?' + re.escape(block_end))
    chunks = []
    cursor = 0
    while cursor < len(source):
        positions = [(source.find(start, cursor), index) for index, start in enumerate(starts)]
        ...
        end = (source.find(ends[kind], content_start) if kind == 2
        ...
        header = header.removeprefix('-').removeprefix('+').removesuffix('-').removesuffix('+').strip()
        words = header.split()
        if kind == 0 and words == ['raw']:
            ...
            lineno = source.count('\n', 0, pos) + 1
            if len(words) == 1:
                raise NoModuleNameDefined('A script block requires a namespace name', lineno, name, filename)
            if len(words) != 2:
                raise TemplateSyntaxError('Expected one script namespace name', lineno, name, filename)
            close = close_script.search(source, after)
            if close is None:
                raise TemplateSyntaxError('Missing endscript tag', lineno, name, filename)
            first_lineno = source.count('\n', 0, after) + 1
            body = source[after:close.start()]
            payload = base64.b64encode(json.dumps([body, first_lineno]).encode()).decode()
            opening_flag = source[content_start:content_start+1]
            opening_flag = opening_flag if opening_flag in ('-', '+') else ''
            closing_content = source[close.start()+len(block_start):close.end()-len(block_end)]
            closing_flag = closing_content[-1:] if closing_content[-1:] in ('-', '+') else ''
            # All original newlines remain inside this replacement tag.
            padding = '\n' * source.count('\n', pos, close.end())
            chunks.append(f'{block_start}{opening_flag} script {words[1]} "{payload}"{padding} {closing_flag}{block_end}')
            cursor = close.end()
            continue
