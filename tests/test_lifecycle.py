...
class LifecycleTests(unittest.TestCase):
    def test_threaded_direct_and_context_import(self):
        ...
        for page in [body+'{{ state.value }}','{% from "lib" import state with context %}{{ state.value }}']:
            with self.subTest(page=page):
                barrier=threading.Barrier(2)
                env=Environment(loader=DictLoader({'lib':body,'page':page}),extensions=[ScriptBlockExtension])
    ...
    def test_threaded_default_import(self):
        env=Environment(loader=DictLoader({'lib':'{% script state %}\nvalues=[]\nbarrier.wait(timeout=5)\n{% endscript %}'}),extensions=[ScriptBlockExtension])
    ...
    def test_bytecode_cache_preserves_freshness_without_compiling(self):
        sources={
            ...
            'wrapper':'{% from "lib" import state %}{% macro count() %}'+COUNT+'{% endmacro %}',
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for cls in [Environment,CachedOnlyEnvironment]:
                env=cls(loader=DictLoader(sources),bytecode_cache=cache,extensions=[ScriptBlockExtension])
...
class AsyncLifecycleTests(unittest.IsolatedAsyncioTestCase):
    def environment(self,sources):
        return Environment(loader=DictLoader(sources),extensions=[ScriptBlockExtension],enable_async=True)
    ...
    async def test_interleaved_async_renders(self):
        ...
        async def synchronize():
            ...
            return ''
        env=self.environment({'lib':'{% script state %}\nvalue=supplied\n{% endscript %}{{ synchronize() }}'})
        env.globals['synchronize']=synchronize
        t=env.from_string('{% from "lib" import state with context %}{{ state.value }}')
        self.assertEqual(await asyncio.gather(t.render_async(supplied=1),t.render_async(supplied=2)),['1','2'])
    ...
    async def test_cancellation_resets_import_observation(self):
        ...
        async def pause():
            ...
            await release.wait()
            return ''
        env=self.environment({'lib':LIB+'{{ pause() }}','pure':'{% set _=ticks.append(1) %}{% macro text() %}ok{% endmacro %}'})
        ...
        async def cancelled_then_continue():
            try:
                ...
                pass
            self.assertEqual([await pure.render_async(),await pure.render_async()],['ok','ok'])
    ...
    async def test_async_template_globals_and_no_context(self):
        env=self.environment({'lib':'{% script s %}\nvalue=supplied\n{% endscript %}','page':'{% from "lib" import s %}{{ s.value }}'})
    ...
    async def test_custom_async_module_creation(self):
        ...
        env=CustomEnvironment(loader=DictLoader({'lib':LIB}),extensions=[ScriptBlockExtension],enable_async=True)
        t=env.from_string('{% from "lib" import state %}'+COUNT)
        self.assertEqual([await t.render_async(),await t.render_async()],['1','1'])
