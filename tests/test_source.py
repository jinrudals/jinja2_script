...
class SourceTests(unittest.TestCase):
    ...
    def test_delimiters_inside_python_strings(self):
        for value in ['{{ untouched }}', '{% if broken', '{# comment #}', '"quotes"', '{% script fake %}']:
            with self.subTest(value=value):
                template = self.env.from_string('{% script text %}\nvalue = '+repr(value)+'\n{% endscript %}{{ text.value }}')
                self.assertEqual(template.render(), value)

    def test_empty_and_comment_only(self):
        for body in ['', '   ', '\n# comment\n', '\n\n']:
            with self.subTest(body=body):
                self.assertEqual(self.env.from_string('{% script empty %}'+body+'{% endscript %}ok').render(), 'ok')

    def test_indentation(self):
        for body in ['value=7', '\n    value = 7\n', '\r\n\tvalue = 7\r\n', '\n\n  # first\n  def f():\n      return 7\n  value = f()\n']:
            with self.subTest(body=body):
                self.assertEqual(self.env.from_string('{% script s %}'+body+'{% endscript %}{{ s.value }}').render(), '7')
    ...
    def test_syntax_error_location(self):
        ...
        with self.assertRaises(CompileError) as raised:
            self.env.compile(source, name='broken.jinja', filename='/templates/broken.jinja')
    ...
    def test_raw_comments_and_jinja_strings(self):
        ...
        self.assertEqual(self.env.from_string(source).render(), '{% script fake %}{% script fake %}')

    def test_nested_jinja_rejected(self):
        with self.assertRaises(TemplateSyntaxError):
            self.env.from_string('{% script bad %}{% if true %}x=1{% endif %}{% endscript %}')

    def test_names_are_per_template(self):
        self.env.from_string('{% script s %}x=1{% endscript %}')
        self.assertEqual(self.env.from_string('{% script s %}x=2{% endscript %}{{ s.x }}').render(), '2')

    def test_custom_delimiters(self):
        for begin,end in [('[%', '%]'), ("<'", "'>")]:
            with self.subTest(begin=begin):
                env=Environment(extensions=[ScriptBlockExtension],block_start_string=begin,block_end_string=end)
                source=begin+' script s '+end+'\nvalue = 7\n'+begin+' endscript '+end+'{{ s.value }}'
    ...
    def test_whitespace_control(self):
        for options in [{}, {'trim_blocks':True,'lstrip_blocks':True}]:
            ...
            self.assertEqual(env.from_string('before \n {%- script s -%}\n x=1\n {%- endscript -%}\n after').render(), 'beforeafter')

    def test_error_after_script_retains_line(self):
        with self.assertRaises(TemplateSyntaxError) as raised:
            self.env.from_string('{% script a %}\nx=1\n{% endscript %}\n{% nonexistent %}')
        self.assertEqual(raised.exception.lineno, 4)
