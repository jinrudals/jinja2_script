...
class RuntimeTests(unittest.TestCase):
    ...
    def test_no_forward_reference(self):
        ...
        t=e.from_string('{{ state.value }}{% script state %}\nvalue=1\n{% endscript %}')
    ...
    def test_templates_do_not_overwrite_each_other(self):
        a=self.env.from_string('{% script helpers %}\nvalue="A"\n{% endscript %}{{ helpers.value }}')
        b=self.env.from_string('{% script helpers %}\nvalue="B"\n{% endscript %}{{ helpers.value }}')
    ...
    def test_runtime_traceback_locates_python_line(self):
        e=Environment(loader=DictLoader({'broken':'text\n{% script work %}\nx=1\nraise ValueError("bad")\n{% endscript %}'}),extensions=[ScriptBlockExtension])
        t=e.get_template('broken')
        try:
            t.render()
        except ValueError as error:
            frames=traceback.extract_tb(error.__traceback__)
            self.assertTrue(any(frame.filename=='broken' and frame.lineno==4 for frame in frames), frames)
        else:
